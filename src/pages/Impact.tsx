        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: '24px', maxHeight: '900px', overflowY: 'auto', padding: '8px' }}>
          {filtered.map((doc, idx) => (
            ...
            >
              ...
              <div style={{ flex: 1 }} />
              <a 
                href={import.meta.env.VITE_STORAGE_BUCKET_URL ? `${import.meta.env.VITE_STORAGE_BUCKET_URL}/${doc.file.replace('/data/', '').replace('/pdfs/uploads/', '')}` : doc.file} 
          ...
          >
            ...
            >
              ...
              <form onSubmit={async (e) => { 
                ...
                const bucketUrl = import.meta.env.VITE_STORAGE_BUCKET_URL;
                if (!bucketUrl) {
                ...
                try {
                  ...
                  const response = await fetch(`${bucketUrl}/${encodeURIComponent(file.name)}`, {
                    method: 'PUT',
                    body: file,
                    headers: {
                  ...
                  alert('Thank you! Your report has been submitted and uploaded successfully.');
                  setShowContribute(false);
                } catch (err) {
                  console.error("Upload error:", err);
                  alert('Failed to upload file. The bucket may not have public write permissions enabled.');
                }
              }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  ...
                  <div style={{ position: 'relative', border: '2px dashed rgba(255,255,255,0.15)', borderRadius: '12px', padding: '32px', textAlign: 'center', cursor: 'pointer', overflow: 'hidden' }}>
                    ...
                    <input type="file" accept=".pdf,.csv" required style={{ opacity: 0, position: 'absolute', inset: 0, width: '100%', height: '100%', cursor: 'pointer' }} />
                  </div>
